...
# This module is based on Hermann Krauss' 2009 GSoC project - 
#       http://wiki.openstreetmap.org/wiki/Srtm2wayinfo
...
class SRTMDownloader:
    ...
    def getTileSet(self,bbox):
        ...
        for x in range(0,nTilesX):
            for y in range(0,nTilesY):
                ...
                print "looking for tile for (%f,%f)\n" % (lon,lat)
                (filename,tile_lon,tile_lat) = self.getTileFname(lon,lat)
                print "filename=%s\n" % (filename)
                if (filename!=None):
    ...
    def getTileFname(self,lon,lat):
        ...
        from the server.
        """
        #To make sure we download the correct tile for negative longitudes, subtract
            # the tile width from negative longitude (1deg)
        if (lon<0): lon = lon-1   
        try:
        ...
        if (filename == None):
            ...
            return (filepath,int(lon),int(lat))
...
class SRTMTile:
    ...
    def __init__(self, f, lat, lon):
        print "SRTMTile.__init__ - file=%s, lat=%d lon=%d" % (f,lat,lon)
